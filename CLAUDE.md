...
- **Build job shows RED on idempotent skips.** When the second cron in a window runs, `recentBuildExists()` returns true, the script exits 0, the "Verify data.json was written" step fails, and the build job is marked failed. Deploy correctly doesn't run. This is **noisy but correct** — Pages keeps the previous deploy. Cleaner fix would set a step output and use `if:` on the verify+upload steps. Unclear — verify with user whether they care about the red-build noise.
